...
public class FileSystem : NSObject {
    ...
    @objc(rename:withName:withResolver:withRejecter:)
    public func rename(source: String, name: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) -> Void {
        guard let index = source.lastIndexOf(value: "/") else {
            reject("ERR", "Failed to rename '\(source)'.", nil)
            return
        }
        let dir = source.substring(startIndex: 0, length: index + 1);
        mv(source: source, target: dir + name, resolve: resolve, reject: reject);
    }
    ...
    @objc(gzipFile:withTarget:withResolver:withRejecter:)
    public func gzipFile(source: String, target: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) -> Void {
        DispatchQueue.global().async {
            let sourceUrl = URL(fileURLWithPath: source.path())
            let targetUrl = URL(fileURLWithPath: target.path())
            do {
                try FileManager.default.zipItem(at: sourceUrl, to: targetUrl)
                resolve(nil)
            } catch {
                reject("ERR", "Failed to zip '\(source)' to '\(target)'. \(error.localizedDescription)", error)
            }
        }
    }

    @objc(unGzipFile:withTarget:withResolver:withRejecter:)
    public func unGzipFile(source: String, target: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) -> Void {
        DispatchQueue.global().async {
            let sourceUrl = URL(fileURLWithPath: source.path())
            let targetUrl = URL(fileURLWithPath: target.path())
            do {
    ...
    @objc(gzipString:withEncoding:withResolver:withRejecter:)
    public func gzipString(data: String, encoding: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) -> Void {
        DispatchQueue.global().async {
            ...
            let fileName = data.md5

            let sourceUrl = cacheURL.appendingPathComponent(fileName + ".source", isDirectory: false)
            let targetUrl = cacheURL.appendingPathComponent(fileName + ".target", isDirectory: false)

            do {
                if encoding == "base64" {
                    guard let decoded = Data(base64Encoded: data, options: .ignoreUnknownCharacters) else {
                        reject("ERR", "Failed to write to '\(sourceUrl.absoluteString)', invalid base64.", nil)
                        return
                    }
                    try decoded.write(to: sourceUrl)
                } else {
                    try data.write(toFile: sourceUrl.path, atomically: false, encoding: .utf8)
                }
                ...
                reject("ERR", "Failed to zip \(error.localizedDescription)", error)
            }
        }
    }

    @objc(unGzipString:withEncoding:withResolver:withRejecter:)
    public func unGzipString(data: String, encoding: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) -> Void {
        DispatchQueue.global().async {
            ...
            do {
                ...
                if encoding == "base64" {
                    ...
                    resolve(String(data: binaryData, encoding: .utf8))
                }
            } catch {
                reject("ERR", "Failed to zip \(error.localizedDescription)", error)
            }
    ...
    @objc(getExternalStoragePaths:withResolver:withRejecter:)
    public func getExternalStoragePaths(removable: Bool, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) -> Void {
        resolve([])
    }
